...
from api.utils.Translation import Names
from api.utils.database import Database
db = Database()

routes = web.RouteTableDef()

@routes.get("/api", allow_head=True)
...
@routes.get("/api/devices", allow_head=True)
async def root_route_handler(_):
    data={}
...
@routes.get("/api/types", allow_head=True)
...
@routes.get("/api/builds", allow_head=True)
async def root_route_handler(request: web.Request):
    device=request.query.get("device")
...
@routes.get("/api/versions", allow_head=True)
async def root_route_handler(request: web.Request):
    data=[]
    device=request.query.get("device")
    build_type=request.query.get("type")
...
@routes.get("/api/file", allow_head=True)
async def root_route_handler(request: web.Request):
    device=request.query.get("device")
    build_type=request.query.get("type")
